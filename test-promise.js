const { lctnScCodes, schulCrseScCodes, SearchSchool, LoginToken, selectGroupList, UserRefresh, Servey } = require('./index-promise');
    ...
    try {
        ...
        if (User.UserInfo.registerDtm != undefined) {
            ...
            if (!sameDay(last, new Date())) {
                servey = await Servey(Token.token);
            }
        } else {
            servey = await Servey(Token.token);
        }
...
if (existsSync("info.json")) {
    ...
    console.clear();
    var lctns = Object.getOwnPropertyNames(lctnScCodes);
    var schul = Object.getOwnPropertyNames(schulCrseScCodes);
            ...
            input.question("학교 이름을 입력해주세요: ", function(answer3) {
                ...
                input.question("이름을 입력해주세요: ", function(answer4) {
                    ...
                    input.question("생년월일을 입력해주세요 (년년월월일일): ", function(answer5) {
                        ...
                        var obj = {
                            lctnScCode: lctnScCodes[lctns[answer1]],
                            schulCrseScCode: schulCrseScCodes[schul[answer2]],
                            schoolName: answer3,
                            Name: answer4,
                            Birth: answer5
                        }
